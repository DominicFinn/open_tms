...
- **IoT Device Tracking** - Admin toggle for IoT vendors (System Loco), then attach one or many tracking devices to a shipment (name + external ID) directly on the shipment form. Inbound device webhooks resolve to the shipment via the device assignment, feeding sensor readings, telemetry, and cold-chain monitoring
