...
export default function TenderDetail() {
  ...
  if (!tender) return <div className="alert alert-error">Tender not found</div>;
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-2)' }}>
            <Link to="/tenders" style={{ color: 'var(--color-text-secondary)', textDecoration: 'none' }}>Tenders</Link>
            <span style={{ color: 'var(--color-text-secondary)' }}>/</span>
            <h1 style={{ margin: 0 }}>{tender.reference}</h1>
            <span className={`chip chip-${statusColors[tender.status]}`}>{tender.status}</span>
            <span className={`chip chip-${tender.strategy === 'broadcast' ? 'info' : 'secondary'}`}>{tender.strategy}</span>
          </div>
          <p style={{ color: 'var(--color-text-secondary)', margin: '4px 0 0' }}>
            <Link to={`/shipments/${tender.shipment.id}`} style={{ color: 'var(--color-primary)' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--spacing-1)' }}>
          {tender.status === 'draft' && (
            <button className="button" onClick={handleOpen} disabled={actionLoading}>
              <span className="material-icons" style={{ fontSize: '18px', marginRight: '4px' }}>send</span>
              Open Tender
            </button>
          )}
          {['draft', 'open', 'evaluating'].includes(tender.status) && (
            <button className="button-danger" onClick={handleCancel} disabled={actionLoading}>Cancel</button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-2)', marginBottom: 'var(--spacing-3)' }}>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--color-text-secondary)', fontSize: '13px' }}>Target Rate</div>
          <div style={{ fontSize: '24px', fontWeight: 700 }}>{tender.targetRate ? `$${tender.targetRate.toLocaleString()}` : '--'}</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--color-text-secondary)', fontSize: '13px' }}>Carriers</div>
          <div style={{ fontSize: '24px', fontWeight: 700 }}>{tender.offers.length}</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--color-text-secondary)', fontSize: '13px' }}>Bids Received</div>
          <div style={{ fontSize: '24px', fontWeight: 700 }}>{tender.bids.filter(b => b.status === 'submitted').length}</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--color-text-secondary)', fontSize: '13px' }}>Lowest Bid</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: lowestBid && tender.targetRate && lowestBid <= tender.targetRate ? 'var(--color-success)' : 'var(--color-text)' }}>
            {lowestBid ? `$${lowestBid.toLocaleString()}` : '--'}
          </div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ color: 'var(--color-text-secondary)', fontSize: '13px' }}>Duration</div>
          <div style={{ fontSize: '24px', fontWeight: 700 }}>{tender.tenderDurationMinutes}m</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-3)', marginBottom: 'var(--spacing-3)' }}>
        <div className="card">
          <h3 style={{ margin: '0 0 var(--spacing-2)' }}>Tender Details</h3>
        ...
        <div className="card">
          <h3 style={{ margin: '0 0 var(--spacing-2)' }}>Timeline</h3>
          <div style={{ display: 'grid', gap: 'var(--spacing-1)', fontSize: '14px' }}>
      ...
      <div className="card" style={{ marginBottom: 'var(--spacing-3)' }}>
        <h3 style={{ margin: '0 0 var(--spacing-2)' }}>Carrier Offers</h3>
            ...
            <tbody>
              {tender.offers.map(o => (
                <tr key={o.id}>
                  ...
                  <td><span className={`chip chip-${statusColors[o.status]}`}>{o.status}</span></td>
                  ...
                  <td>{o.ediSent ? <span className="chip chip-success">Sent</span> : '--'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Bids */}
      <div className="card">
        <h3 style={{ margin: '0 0 var(--spacing-2)' }}>Bids</h3>
        {tender.bids.length === 0 ? (
          <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', padding: 'var(--spacing-3)' }}>No bids received yet</p>
        ) : (
          <div className="table-container">
            <table className="data-table">
              <thead>
                ...
                {tender.bids.map(b => {
                  ...
                  return (
                    <tr key={b.id}>
                      ...
                      <td>
                        {diff !== null && (
                          <span style={{ color: diff <= 0 ? 'var(--color-success)' : 'var(--color-error)', fontWeight: 600 }}>
                      ...
                      <td>
                        <span className={`chip chip-${b.sourceType === 'edi_990' ? 'info' : 'secondary'}`}>
                          {b.sourceType === 'edi_990' ? 'EDI' : b.sourceType}
                        </span>
                      </td>
                      <td><span className={`chip chip-${statusColors[b.status]}`}>{b.status}</span></td>
                      ...
                      <td>
                        ...
                        {b.status === 'accepted' && (
                          <span style={{ color: 'var(--color-success)', fontWeight: 600 }}>Winner</span>
                        )}
                      </td>
                    </tr>
                  );
      ...
      {showAwardModal && (
        <div className="modal-backdrop" onClick={() => setShowAwardModal(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: '450px' }}>
            <h3 style={{ margin: '0 0 var(--spacing-2)' }}>Award Tender</h3>
            <p>
            ...
            <div className="form-actions">
              <button className="button-outline" onClick={() => setShowAwardModal(null)}>Cancel</button>
              <button className="button-success" onClick={() => handleAward(showAwardModal)} disabled={actionLoading}>
                Confirm Award
              </button>
            </div>
          </div>
