...
import { getInventoryAppToken, getInventoryAppUser, saveInventoryAppSession } from './inventory-session';

/**
 * Inventory-app login (#233): reuses the warehouse PWA's magic-link + password login
 * endpoints, requesting `scope: 'inventory'` so the backend mints the narrower session JWT
 * (read levels + record observations only — see backend/src/middleware/jwtAuth.ts).
 */
export default function InventoryAppLogin() {
  ...
  useEffect(() => {
    if (getInventoryAppUser() && getInventoryAppToken()) {
      const loc = localStorage.getItem('inventory_app_location');
      navigate(loc ? '/inventory-app' : '/inventory-app/select-location');
    }
  }, [navigate]);
  ...
  async function handlePasswordLogin(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/warehouse/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password, scope: 'inventory' }),
      });
      const json = await res.json();
      if (!res.ok || json.error) {
  ...
  function loginSuccess(token: string, user: any) {
    saveInventoryAppSession(token, user);
        ...
        <CardHeader className="items-center text-center">
          <div className="mb-2 flex flex-col items-center gap-4">
            ...
            <h1 className="text-2xl font-bold tracking-tight">
              Inventory <GradientText>App</GradientText>
            </h1>
            <p className="text-sm text-muted-foreground">Sign in to check stock and record observations</p>
          </div>
        </CardHeader>
        <CardContent>
          {error && (
          ...
          <form onSubmit={handlePasswordLogin} autoComplete="on" className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="inv-email" className="text-base">Email</Label>
              <Input
                id="inv-email"
                type="email"
                value={email}
            ...
            <div className="space-y-2">
              <Label htmlFor="inv-password" className="text-base">Password</Label>
              <Input
                id="inv-password"
                type="password"
                value={password}
